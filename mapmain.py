...
def clean_api_code():
    ...
    with open("SurveyDataMarch3.json",encoding="utf8") as file:
        data = json.load(file)
    survey_df = json_normalize(data['results'])

    with open("HistoryEnvironmentalHealthMarch3.json",encoding="utf8") as file:
        data = json.load(file)
    environmental_df = json_normalize(data['results'])
    ...
    ReClean NaNs
    '''
    survey_df = survey_df.replace({pd.np.nan: ''})
    ...
    #Clean Environmental
    environmental_df = environmental_df.replace({pd.np.nan: ''})
    environmental_df = environmental_df.drop_duplicates(subset=['client.objectId'])

    #Clean more
    survey_df_nulls = survey_df.replace({'':pd.np.nan})
    environmental_df_nulls = environmental_df.replace({'':pd.np.nan})
    ...
    full_df = full_df.replace({pd.np.nan: ''})
    ...
    educationLevel_vals = ['','lessThanprimary', 'primary', 'college' ,'highschool', 'someHighSchool',
     'someCollege']
    educationLevel_vals_new = ['','Less Than Primary School', 'Completed Primary School', 'Completed College' ,'Completed High School', 'Some High School',
     'Some College']
    ...
    map_columns = ['objectId', 'educationLevel', 'latitude', 'longitude', 'age', 'communityname','city','city_match', 'waterAccess', 
           'clinicAccess', 'conditionoFloorinyourhouse', 'conditionoRoofinyourhouse',  'stoveType', 'houseMaterial', 'electricityAccess',
           'foodSecurity', 'govAssistance', 'numberofIndividualsLivingintheHouseDigits','Latrine or Bathroom Access']

    rename_columns = ['objectId', 'Education Level', 'Latitude', 'Longitude', 'Age','Community','City', 'City NLP Match', 'Water Access', 
           'Clinic Access', 'Floor Condition', 'Roof Condition', 'Stove Ventilation', 'House Material', 'Electricity Access',
           'Food Security', 'Government Assistance', 'Number of People in the House', 'Latrine or Bathroom Access']
...
app.layout = html.Div([html.Div([html.H1("Puente Insights")],
                       ...
                       html.Div([html.Span("Metric to display : ", className="two columns",
                                           style={"text-align": "right", "width": "40%", "padding-top": 10}),
                                 dcc.Dropdown(id="value-selected", value='Confirmed',
                                              options=[{'label': "Age ", 'value': 'Age'},
                                                       {'label': "Number of People in the House", 'value': 'Number of People in the House'}],
...
def update_figure(selected):
    ...
    dff = clean_api_code()
    dff['hover_text'] = dff[selected].apply(str)
