...
app.layout = html.Div([
    ...
    html.Hr(),
    html.Div([
    html.Label(["Multiselect Feature Filter",dcc.Dropdown(id='options-dropdown',
    multi=True,
    style={'size':'20%'},
 ...
 'Completed High School', 'Some High School',  'Some College']
    ...
    html.Label(["Community",dcc.Dropdown(
    id = 'community-selection',
    options=[
        {'label': x.capitalize(), 'value': x} for x in
                          sorted(df['Community (Clean)'].unique())],value='Arenazo')]),

    html.Div([
        dcc.Graph(id='display-selected-values',figure={})
    ])
],style={'width':'30%','margin-right':'90%','display':'flex','margin-left':'auto','margin-right':'0'})
...
@app.callback(
    ...
    Input('community-selection','value'))

def set_display_children(selected_feature, selected_option,selected_community):
    dff = df[df[selected_feature].isin(selected_option)]
    dff = dff[dff['Community (Clean)']==selected_community]
    ...
    fig.update_layout(mapbox_style="mapbox://styles/msuarez9/ckmgrly42280417piotkluhrh")
    fig.update_layout(
        hoverlabel=dict(
            bgcolor="white",
            font_size=16,
