...
def clean_api_code():
    ...
    #Filter latitude and longitude that are out of range
    cleandf2 = cleandf2[cleandf2['Latitude']!='']

    latmin=17
    latmax=20
    lonmin=-72
    lonmax=-68
    cleandf2= cleandf2[cleandf2['Latitude'].astype(float)>latmin]
    cleandf2= cleandf2[cleandf2['Latitude'].astype(float)<latmax]
    cleandf2= cleandf2[cleandf2['Latitude'].astype(float)>lonmin]
    cleandf2= cleandf2[cleandf2['Latitude'].astype(float)<lonmax]
...
app.layout = html.Div([
    ...
    id = 'city-selection',
    options=[
        {'label': x.capitalize(), 'value': x} for x in
                          sorted(df['Community (Clean)'].unique())],value='San pedro de macorís')
])

@app.callback(
    Output('display-selected-values','figure'),
    [Input('clinicaccess-dropdown','value'),Input('city-selection','value')]
)

def update_output(clinicvalue,cityvalue):
    dff = df[df['Clinic Access'].isin(clinicvalue)]
    dff = dff[dff['Latitude']!=''] 
    dff = dff[dff['City NLP Match']==cityvalue]
