...
app.layout = html.Div([
    ...
    options=[
        {'label': x.capitalize(), 'value': x} for x in
                          sorted(df['Community (Clean)'].unique())],value='Arenzano')
])

@app.callback(
    Output('display-selected-values','figure'),
    [Input('clinicaccess-dropdown','value'),Input('community-selection','value')]
)

def update_output(clinicvalue,communityvalue):
    dff = df[df['Clinic Access'].isin(clinicvalue)]
    dff = dff[dff['Latitude']!=''] 
    dff = dff[dff['Community (Clean)']==communityvalue]
